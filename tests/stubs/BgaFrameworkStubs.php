...
abstract class Table
{
    ...
    public static function getObjectListFromDB(string $sql, bool $bUniqueValue = false): array
    {
        ...
        if (preg_match("/^SELECT (.*) FROM `cards` WHERE `player_id` = (?P<player_id>.*) AND `subtype` = (?P<subtype>.*) AND `card_location` = '(?P<card_location>.*)'( ORDER BY card_state (ASC|DESC))?$/", $sql, $matches) == 1) {
            //TODO SORT card_state
            $card_location = $matches['card_location'];
            $player_id = $matches['player_id'];
            $subtype = $matches['subtype'];
            $filtered = array_filter(TestDatas::$cards,function ($card) use ($card_location, $player_id, $subtype){return $card['card_location'] == $card_location && $card['subtype'] == $subtype && $card['player_id'] == $player_id;});
            return $filtered;
        }

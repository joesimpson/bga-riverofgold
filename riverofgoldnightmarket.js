...
function (dojo, declare, BgaAnimations, BgaDice) {
    ...
    const MASTERY_TYPE_WAVES     = 7;
    const MASTERY_TYPE_SUN_MOON  = 8;
        ...
        */
        ...
        setup: function( gamedatas )
        {
            ...
            if(this.gamedatas.city_of_lies){
                toPreloadList.push('cityOfLies/board_city.webp');
                ...
                if( Object.keys(args.possibleCityCards).length > answeredCardsIds.length){
                    ...
                    for (const [cId, cardDatas] of Object.entries(args.possibleCityCards)) {
                        if(! answeredCardsIds.includes(cId)){
                            ...
                            await this.bga.dialogs.multipleChoice(this.fsr(_('Do you want to play your ${card_name} ?'), {'card_name': _(cardDatas.name)}), choices).then(choice => {
        ...
        notif_revealCityCard(n) {
            ...
            let card = n.args.card;
            this.removeCityCardBackInPlayerHand(card.pId, card.state);
            let cardDiv = this.addCard(card);
            this.animationManager.slideAndAttach(cardDiv, this.getCardContainer(card), {duration: 700})
                .then(() => {
                });
        ...
        togglePinMode(check){
            let newValue = check ? true : false;
            this.sidePanelPinned = newValue;
            $('ebd-body').setAttribute('data-rog_pin_panel', newValue);
            //$('pin-panel-switch').setAttribute('data-pinned', newValue);
        },
        ...
        format_string_recursive(log, args) {
            ...
            if (log && args && !args.processed) {
                ...
                if('card_icon' in args && 'inner' in args) {
                    args.card_icon = this.formatCityCardBackIcon(args.inner,'_log');
                } else if('card_icon' in args && args.card.subtype == CARD_TYPE_CITY) {
                    args.card_icon = this.formatCityCardIcon(args.card,'_log');
                }
        ...
        setupAutomaPanel(){
            ...
            if(!automa_panel){
                this.bga.playerPanels.addAutomataPlayerPanel(
                    automa_id, 
                    _(automa_player.name) + " : "+ _(this.gamedatas.automa_level_name),
                    {
        ...
        setupInfoPanel() {
            ...
            //$('pin-panel-switch').addEventListener('click', () => {
            //   let oldValue = this.settings.sidePanelPinned;
            //   let newValue = (oldValue == 1) ? 0 :1;
            //   this.settings.sidePanelPinned = newValue;
            //   $('ebd-body').setAttribute('data-rog_pin_panel', newValue);
            //   $('pin-panel-switch').setAttribute('data-pinned', newValue);
            // });
        ...
        tplConfigPlayerBoard() {
            ...
            <div class='player-board' id="player_board_config">
                ...
                <div class="player_config_row">
                    ...
                    <div id="pin-panel-switch">
                        <input type="checkbox" class="checkbox" id="pin-panel-chk" />
                        <label class="label" for="pin-panel-chk">
                            <div class="ball"></div>
                        </label>
                        <div class='pin-panel-icon'><i class="fa6 fa6-thumbtack"></i></div>
                    </div>
        ...
        tplClanCard(card, card_side = 0, prefix ='') {
            let descriptionMap = new Map([
                ...
                [PATRON_SCION_OF_EARTH      , this.fsr(_('When you claim a mastery, ${resource} and gain ${n} ${favor}. You may claim a mastery even if all its ${score} rewards have been claimed (if they have, put a clan marker on it but do not earn any ${score}).'),{ 'resource':this.formatIcon("moon_up"),'n':3, 'favor':this.formatIcon(RESOURCES[RESOURCE_TYPE_SUN]),'score':this.formatIcon('score'), })],
                ...
                [PATRON_LIONS_LADY          , this.fsr(_('During setup, place the ${lion} token on an empty shore space. Other players cannot build there. When you build there, gain the building\'s owner reward or visitor reward, then move the token to a new empty shore space.'),{'lion': this.formatIcon('bonus-'+BONUS_TYPE_PLACE_LION) })],
                ...
                [PATRON_TATTOOED_MONK       , this.fsr(_('When you discard a customer (after delivering or ${draw}), gain ${influence} in the customer\'s region.'),{ 'draw': this.formatIcon('bonus-'+BONUS_TYPE_DRAW),'influence':this.formatIcon("influence"), 'n2': 1})],
        ...
        removeCityCardBackInPlayerHand(cardPid, cardState ){
            let cardSubtype = CARD_TYPE_CITY;
            let cardLoc = CARD_CITY_LOCATION_HAND;
            let div = document.querySelector(`.rog_city_card[data-pid="${cardPid}"][data-subtype="${cardSubtype}"][data-state="${cardState}"][data-location="${cardLoc}"]`);
            if(div){
                this.destroy(div);
            }
        },
        ...
        tplCityCardBack(card, prefix ='') {
            return `<div class="rog_card rog_card${prefix} rog_city_card rog_city_card_back" id="rog_city_card_back${prefix}-${card.id}" 
        ...
        tplCityCard(card, prefix ='') {
            return `<div class="rog_card rog_card${prefix} rog_city_card" id="rog_card${prefix}-${card.id}" 
                    data-inner="${card.inner}"
                    data-type="${card.type}" 
                    data-subtype="${CARD_TYPE_CITY}"
                    data-state="${card.state}"
        ...
        formatCityCardIcon(card, prefix =''){
            return this.tplCityCard(card,prefix);
        },
        formatCityCardBackIcon(isInner, prefix =''){
